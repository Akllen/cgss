...
func Login(args []string) int {
	...
	if err != nil {
		fmt.Println("Invalid Parameter:<level>shoud be an integer.")
		return 0
	}
	exp, err := strconv.Atoi(args[3])
	if err != nil {
		fmt.Println("Invalid Parameter:<exp>shoud be an integer.")
	}
	player := cg.NewPlayer()
	player.Name = args[1]
	player.Level = level
	...
	err = centerClient.AddPlayers(player)
	if err != nil {
